...
export default function NervesLeague() {
  ...
  var userData = location.state.selectedPlayer;
  ...
  if (userData) {
    userData.map((player) => {
      totalPoints = totalPoints + Number(player.pointsearned);
    });
  }
  // useEffect(() => {
  //   getCurrentDate();
  ...
  const handleSubmit = async () => {
    ...
    if(data[0].Teamcode !== null){
      console.log("dar",data[0].Teamcode);
            ...
            {userData &&
              userData.map((player, index) => (
                ...
                >
                  ...
                  <div className="points pe-3">{player.pointsearned}</div>
                </div>
              ))}
