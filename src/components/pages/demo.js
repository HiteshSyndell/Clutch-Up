...
const LandingScreen = (props) => {
  ...
  const test = async () => {
    const { data } = await supabase.from("playerlist").select().order('id', { ascending: true }).range(0, 50);
    setPlayersdata(data);
  ...
  useEffect(async () => {
    ...
    if (day === 'Tuesday') {
      ...
      axios("https://fortniteapi.io/v1/events/list?lang=en&region=NAE", { method: 'GET', headers: { Authorization: '04876d40-df9ddb98-13a88349-fe7809ed' } })
        .then((responses) => {
          ...
          responses.data.events.map(async (elems) => {
            if (elems.id === "epicgames_AlphaTournamentNA") {
              ...
              for (let ind = 0; ind < myEndPoints.length; ind++) {
                ...
                axios(`https://fortniteapi.io/v1/events/window?windowId=${myEndPoints[ind].windowId}`,
                  { method: 'GET', headers: { Authorization: '04876d40-df9ddb98-13a88349-fe7809ed' } })
                  .then((response) => {
                    ...
                    console.log(myEndPoints[ind].windowId === stering[0] + "_Round2");
                    if (myEndPoints[ind].windowId === stering[0] + "_Round2") {
                      ...
                      for (let index = 0; index < response.data.session.results.length; index++) {
                        ...
                        for (let i = 0; i < response.data.session.results[index].teamAccountNames.length; index++) {
                          ...
                          var name2 = response.data.session.results[index].teamAccountNames[1].name;
                          var combineName = name1 + " & " + name2
                          Playersdatas.push({ name: combineName, price: '5', score: '0' })
  ...
  useEffect(() => {
    ...
    const getdata = async () => {
      const apicall = await axios(`https://fortniteapi.io/v1/events/list?lang=en&region=NAE`,
        { method: 'GET', headers: { Authorization: '04876d40-df9ddb98-13a88349-fe7809ed' } })
    ...
    axios(api,
      ...
      .then((data) =>
        !data ?
          setLoading(true)
          :
          setPersons([data.results.slice(0, 10)]))
  }, [player])
  ...
  useEffect(() => {
    axios(api,
      ...
      .then((data) => {
        ...
        let new_data = filterdata.map((e, i) => { return e.teamAccountNames })
        const final = new_data.map((e, i) => { return e })
        setPlayer(final)
      })
  }, [player])
  ...
  const tournamentlistSupabase = async () => {
    const { data, error } = await supabase
      .from("tournamentlist")
      .insert({
        tournamentname: testPlayer[0].eventId,
        tournamentstartdate: testPlayer[0].beginTime,
        tournamentenddate: testPlayer[0].endTime,
  ...
  const eventslistSupabase = async () => {
    const { data, error } = await supabase
      .from("eventslist")
      .insert({
        eventsname: testPlayer[0].windowId,
        // tournamentid: 1,
        playersnumber: player_num,
        startdate: testPlayer[0].beginTime,
        enddate: testPlayer[0].endTime,
        createddate: new Date(),
        modifieddate: new Date()
      })
            ...
            <div className="contentrow col-12">
              <div className="nevcolumn col-2">
                ...
                <div className="nevlistst">
                  ...
                  <li className="listnevbars"><a className="tournament-list-admin-penal" onClick={selcet_list}>Selection List</a></li>
                  <li className="listnevbars"><a className="tournament-list-admin-penal" onClick={tours_list}>Tournaments List</a></li>
                  <li className="listnevbars"><a className="tournament-list-admin-penal" onClick={events_list}>Events List</a></li>
                  <li className="listnevbars"><a className="tournament-list-admin-penal" onClick={players_list}>Players List</a></li>
                  ...
                  <div className="landing-box col-12">
                    ...
                    <div style={{ overflowy: 'auto' }}>
                      <div className="syncname">
                        <button className="syncbutton" onClick={apicall}>sync</button>
                      </div>
                      <div>
                      </div>
                    </div>
                    ...
                    {
                      jsonResult.filter((e, i) => e.id === tourcng).map((e, i) => {
                        mywindows = e.windows
                        ...
                        < div className="d-flex justify-content-center align-items-center flex-column">
                          <Stack sx={{ width: 400, margin: "auto" }}>
                            <Autocomplete
                              ...
                              renderInput={(params) => <TextField {...params} label="Select Your Tournament..." />}
                            />
                            {
                              jsonResult.filter((e) => e.id === tourcng).map((e, i) => {
                                return (
                                  <>
                              ...
                              <>
                                <Stack sx={{ width: 400, margin: "auto" }}>
                                  <Autocomplete
                                    disablePortal
                                    id="nba_player_demo"
                                    getOptionLabel={(mywindows) => `${mywindows.windowId}`}
                                    options={mywindows}
                                    sx={{ width: 400 }}
                                    isOptionEqualToValue={(option, value) =>
                                    ...
                                    renderOption={(props, mywindows) =>
                                    (
                                      <Box component="li" {...props} key={mywindows.windowId}>
                                        {mywindows.windowId}
                                      </Box>
                                    )}
                                    renderInput={(params) => <TextField {...params} label="Select Your Event!..." />}
                                  />
                                  {
                                    mywindows.filter((e) => e.windowId === wincng).map((e, i) => {
                                      return (
                                        <>
                                          <div className="alert alert-primary mt-2" role="alert">
                                            Begin Time:- {e.beginTime}
                          ...
                          <div className="d-flex flex-column mt-5">
                            <label htmlFor="player" className="my-2">Please Enter The Players Number!</label>
                            <input type="text" className="form-control w-100 mb-2" value={player_num} onChange={(e) => setPlayer_num(e.target.value)} placeholder="Number Of Players!" id="player" />
                          </div>
                          <button className="btn btn-primary" onClick={PlayerNum}>Save</button>
                        </div>
                        :
                        ''
                    }
                    ...
                    {tour_List ?
                      <>
                        {tourId === undefined ? <h1 className="text-center my-5">Loading...</h1> :
                          <>
                                      ...
                                      return (
                                        <>
                                          <tr key={i + 1}>
                                            ...
                                            {/* <td>eventId</td> */}
                                            <td>{e.beginTime}</td>
                                            {/* <td>{e.sessionHistory[0].endTime}</td> */}
                                            <td>{e.endTime}</td>
                                            <td>Created Date</td>
                                            <td>Modified date</td>
                                          </tr>
                            ...
                            <div className="table-responsive">
                              ...
                              <table className="table table-hover">
                                ...
                                <tbody>
                                  ...
                                  {
                                    apifunc.slice(start, player_num).map((e, i) => {
                                      // console.log("tabledata", e)
                                      return (
                                        <>
                                          <tr key={i + 1}>
